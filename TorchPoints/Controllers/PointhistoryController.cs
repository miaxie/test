...
namespace TorchPoints.Controllers
{
    ...
    public class PointhistoryController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public ApiResponseModel<PointHistory> Post([FromBody] PointHistoryModel history)
        {
            ...
            if (added==null)
            {
                return new ApiResponseModel<PointHistory>(Model.StatusCode.HandledFaild,"保存失败，请稍后重试");
            }
            return new  ApiResponseModel<PointHistory>(added);
        }
    }
}